...
class GNNPolicy(nn.Module):
    ...
    - a configurable data encoder (bipartite / sage) for MILP bipartite graphs,
...
class HeuristicPolicy(torch.nn.Module):
    ...
    def init_embedding(
        ...
        if branching_candidates.dim() != 2:
            raise ValueError("branching_candidates must be rank-2 tensor: (B, T).")
        ...
        x_parent = torch.cat((parent_flags, x_base[:, :1]), dim=-1)
...
# class HeuristicPolicy(torch.nn.Module):
    # """
    # A heuristic-inspired GNNPolicy

    # 1. oracle or heuristic (customized get_nodes_to_branch method) to 
    # select n_branching n_branching candidates, say T in total
    
    # 2. pre-create (B, T, 2, N, F) features, T represents branching, 2 represents 2 subproblems via branching
    # Y: (B, T, 2, n_constraints, F)
    # X: (B, T, 2, n_variables, F)

    # 3. add one-hot encodings to X
    # X: (B, T, 2, n_variables, F+2) such that 
    # X[b, t, :, :, :] = [[X, 1_v, 0], [X, 0, 1_v]] where we use 1_v to distinguish T branchings

    # use n_constraints_per_graph, n_variables_per_graph and torch geometric's to_dense_batch to create
    # real_x_mask and real_y_mask, this should facilitate batched GNN operations

    # 4. 

    # 4. GNN(Y, edge_indices, edge_features, X)
