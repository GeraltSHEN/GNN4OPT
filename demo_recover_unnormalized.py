...
def _select_sample_paths(
    ...
    end_index = min(sample_index + max_samples, len(split_files))
    return [Path(p) for p in split_files[sample_index:end_index]]
...
def main() -> None:
    ...
    parser.add_argument("--sample_index", type=int, default=0, help="Start index in split")
    parser.add_argument("--max_samples", type=int, default=1, help="Max number of samples to process from sample_index")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Raise error if |pseudo_score-orig_score| exceeds this for printed top-k pseudo rows",
    )
    args = parser.parse_args()

    sample_paths = _select_sample_paths(
        args.dataset_path,
        args.split,
        args.sample_index,
        args.max_samples,
    ...
    for i, sample_path in enumerate(sample_paths):
        sample_idx = args.sample_index + i
        ...
        if context.lp.row_duals is not None and context.lp.reduced_costs is not None:
            ...
            dual_rc_gap = abs(obj_dual_rc - obj_from_state)
            print(f"objective_from_dual_plus_rc:  {obj_dual_rc:.12g}")
            print(f"dual_rc_gap_vs_primal:        {dual_rc_gap:.3e}")
        else:
        ...
        sb_rank_match = sb_topk["topk_rank_original"] == sb_topk["topk_rank_computed"]
        print(f"sb_rank_match: {sb_rank_match}")
        ...
        if dual_rc_gap is not None and dual_rc_gap > args.dual_rc_gap_tol:
            raise RuntimeError(
                f"[sample_index={sample_idx}] dual_rc_gap_vs_primal={dual_rc_gap:.6e} "
                f"> dual_rc_gap_tol={args.dual_rc_gap_tol:.6e}"
            )

        if not sb_rank_match:
            raise RuntimeError(
                f"[sample_index={sample_idx}] sb_rank_match=False "
                f"(original={sb_topk['topk_rank_original']}, reconstructed={sb_topk['topk_rank_computed']})"
            )

        for pos in top_pseudo_positions.tolist():
            orig_score = float(anchor_res["candidate_scores"][pos])
            pseudo_score = float(anchor_res["pseudo_scores"][pos])
            gap = abs(orig_score - pseudo_score)
            if gap > args.pseudo_score_tol:
                raise RuntimeError(
                    f"[sample_index={sample_idx}] pseudo_score mismatch at cand_pos={pos}: "
                    f"orig_score={orig_score:.12g}, pseudo_score={pseudo_score:.12g}, "
                    f"gap={gap:.6e} > pseudo_score_tol={args.pseudo_score_tol:.6e}"
                )
